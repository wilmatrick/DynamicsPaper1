...
\subsubsection{Preparation: Tracer density}
...
&\approx& \int_{-4\sigma_R(R|p_{DF})}^{4\sigma_R(R|p_{DF})} \int_{-4\sigma_z(R|p_{DF})}^{4\sigma_z(R|p_{DF})} \int_{0}^{1.5 v_{circ}(R_\odot)}   \text{qDF}(J[R,z,\vect{v}|p_{\Phi}]|p_{DF}) \diff v_T \diff v_z \diff v_R,
\end{eqnarray*}
where $\sigma_R(R|p_{DF})$ and $\sigma_R(R|p_{DF})$ are given by eq. (???) and (???). The size of the $N_\text{spatial} \times N_\text{spatial}$ grid in $(R,z)$ is chosen cover the extent of the observed volume for $z>0$. We interpolate then over this grid to be able to evaluate the density at each $(R,z)$ within the observed volume. Fig. ??? shows the importance of choosing $N_\text{spatial}$ and $N_\text{velocity}$ sufficiently large.
