...
\caption{Caption [TO DO] (This was done using the current qDF to set the fitting range. Nvelocity=24 and Nsigma=5 is high enough (though not perfect). Maybe redo with fiducial qD. [TO DO] Also compare these results with the result when marginalizing over $vT$ [TO DO] Colors are wrong [TO DO])} 
\end{figure}

\begin{figure}
\plotone{~/BovyCode/analyzeFlexible/plots/isoSphFlexIncompR/isoSphFlexIncomp_mockdata.eps}
\caption{Caption [TO DO] (Re-do, if new analyses are in violin plot.)} 
\end{figure}
