...
[TO DO: Make sure that probabilities have lowercase $p$ everywhere.]
...
\begin{figure} [t!]
  \caption{(Continued.) We calculate the true normalization with high accuracy as $M_\text{tot,true} \approx M_\text{tot}(N_\text{spatial}=20,N_\text{velocity}=56,N_\text{sigma}=7)$. The dashed lines indicate the accuracy used in our analyses: it is better than $0.002\%$ for all three potential types. Only for the smallest volume in the \texttt{MW13-Pot} (yellow line) the error is only $\sim 0.005\%$. This could be due to the fact, that, while we have analytical formulas to calculate the actions for the isochrone and the Staeckel potential exactly, we have to resort to an approximate action calculation for the MW-like potential (see Section \ref{sec:potentials}). [TO DO: Try to redo yellow curve in MW. Weird, that it does not depend on $N_{spatial}$.???] [TO DO: fancybox Legend] [TO DO: Potential and/or population names in typewriter font]}
\end{figure}
