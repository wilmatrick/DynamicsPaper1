...
\NEW{(ii)}  Modest systematic differences between the true and \NEW{best fit}\OLD{best-fit} model DF are inconsequential. \NEW{For example, when} \OLD{When} defining sub-populations by binning stars according to their chemical abundances, finite bin sizes and abundance errors do not affect the modelling as long as the DF parameters of neighbouring bins do not differ by more than $20\%$.
...
on parameter recovery 	& \emph{Survey volume:}	& 4 different wedges, see Figure \ref{fig:wedFlexVol_bias_vs_SE}, upper panel & (fixed \& known) & \\
						& \emph{$N_{*}$:} & 20,000 & & \\
\tableline
Test \testlabel{test:isoSphFlexIncomp}{5} :        & \emph{Potential:}     & \OLD{\texttt{Iso-Pot}}\NEW{\texttt{DHB-Pot}} & \OLD{\texttt{Iso-Pot}, all parameters free}\NEW{\texttt{DHB-Pot}, free parameters:} & Figures \ref{fig:MWdhbIncompR_violins} \& \ref{fig:isoSphFlexIncompR_marginal_violins}\\
Influence of & & & \NEW{$v_\text{circ}(R_\odot), a_\text{disk}, f_\text{halo}$} &\\
wrong assumptions            & \emph{DF:}          & \texttt{hot} or \texttt{cool} qDF & qDF, all parameters free &   \\
about the spatial SF        & \emph{Survey volume:} & sphere around Sun, $r_\text{max} = 3~\text{kpc}$ & (fixed \& known) & \\
on parameter recovery    & \emph{Completeness:}  & Equation \eqref{eq:rad_incomp}   & completeness$(\vect{x})$ = 1, & \\
       &                       &  with $\epsilon_r \in [0,0.7]$ & i.e., $\epsilon_r=0$& \\
   &   \emph{$N_{*}$:}                     & 20,000  & & \\
\tableline
...
\subsection{\NEW{Selection functions}} \label{app:selectionfunction}%\label{sec:selectionfunction}
