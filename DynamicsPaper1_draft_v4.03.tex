...
Influence of survey & \emph{Survey volume:} & sphere around Sun, $r_\text{max} = 0.2, 1, 2, 3$ or $4~\text{kpc}$ & (fixed \& known) & \NEW{estimator.} \Wilma{[TO DO: Volume result]}\\
...
Influence of 			& 						& 													& (ii) \OLD{\texttt{MW13-Pot}, $R_d$ and $f_h$ free}\NEW{\texttt{DHB-Pot}, free parameters:} & \\
position \& shape 		& 			&  										& \NEW{$v_\text{circ}(R_\odot)$, $a_\text{disk}$, $f_\text{halo}$}& \\
of survey volume 		& \emph{DF:} & \texttt{hot} qDF											& \OLD{(i) }qDF, all parameters free\OLD{(ii) qDF, only $h_R$, $\sigma_{z,0}$ and $h_{\sigma,R}$ free}& \\
