...
%% Abbreviations
\shorttitle{Action-based Dynamical Modeling for the Milky Way Disk}
...
\tablecaption{Axisymmetric gravitational potential models used throughout this work. \label{tbl:referencepotentials}}
\tablewidth{0pt}
\tablehead{
\colhead{Name} & \colhead{Potential Model} & \multicolumn{2}{c}{Parameters $p_\Phi$} & \colhead{Action Calculation}}
\startdata
\texttt{Iso-Pot} & isochrone potential\tablenotemark{(a)} & $b$& $0.9~\text{kpc}$ & \emph{analytic and exact} \\
 & \citep{1959AnAp...22..126H} & & & (\citealt{2008gady.book.....B}, \S 3.5.2) \\
\tableline
\texttt{KKS-Pot} & 2-component & $\Delta$ & $0.3$ & \emph{exact} \\
 ...
 & & & & \\
 \tableline
\texttt{DHB-Pot} & Disk+Halo+Bulge potential\tablenotemark{(c)}: & $a_\text{disk}$ & $3~\text{kpc}$ & \emph{approximate} \\ 
...
%====================
\begin{figure*}[!htbp]
\includegraphics[width=\textwidth]{figs/reference_potentials-eps-converted-to.pdf}
...
\tabletypesize{\scriptsize}
\tablecaption{Reference Parameters for the qDF in Equations \eqref{eq:df_general}-\eqref{eq:sigmazRg}. \label{tbl:referenceMAPs}}
\tablewidth{0pt}
\tablehead{
...
\enddata
\tablecomments{These parameters are used to create 6D phase-space mock data sets for stellar populations of different kinematic temperature. The parameter $X$ is explained in Section \ref{sec:qDF}.}
\end{deluxetable*}
%====================
...
\begin{figure*}[!htbp]
\centering
\plotone{figs/likelihood_normalisation_accuracy-eps-converted-to.pdf}
\caption{Relative error of the likelihood normalization, $\delta M_\text{tot}$, in Equation \eqref{eq:relerrlikelihood}, depending on the accuracy of the grid-based density calculation in Equation \eqref{eq:tracerdensity} (and surrounding text) in five spherical observation volumes with different radius, $r_\text{max}$, and depending on the kinematic temperature of the population in the \texttt{DHB-Pot}. (Test \ref{test:norm_accuracy} in Table \ref{tbl:tests} summarizes the model parameters.) The tracer density in Equation \eqref{eq:tracerdensity} is calculated on $N_x\times N_x$ spatial grid points in $R \in [R_\odot \pm r_\text{max}]$ and $|z| \in [0,r_\text{max}]$. The integration over the velocities is performed with Gauss-Legendre quadratures of order $N_v$ within an integration range of $\pm n_\sigma$ times the dispersion $\sigma_R(R)$ and $\sigma_z(R)$ (and $[0,1.5v_\text{circ}]$ in $v_T$). (We vary $N_x$, $N_v$ and $n_\sigma$ separately, and keep the other two fixed at the values indicated above each panel.) We calculate the ``true'' normalization $M_\text{tot}$ in Equation \eqref{eq:relerrlikelihood} with high accuracy as $M_\text{tot} \equiv M_\text{tot,approx}(N_x=32,N_v=68,n_\sigma=7)$. The black stars indicate the accuracy used in analyses with the \texttt{DHB-Pot}, Tests \ref{test:isoSphFlexIncomp} and \ref{test:MWdhbMix}. It is better than $0.005\%$ (dotted line), which is required for $N_*=20,000$ stars. We find that the spatial resolution of the grid is important and depends on the kinematic temperature of the population, as cooler populations have a steeper density gradient in the $z$-direction, which has to be sampled sufficiently.}
\label{fig:norm_accuracy}
\end{figure*}
%====================

\subsection{Likelihood Normalization} \label{sec:likelihood_normalisation}

The normalization in Equation \eqref{eq:prob} is a measure for the total number of tracers inside the survey volume,
\begin{equation}
M_\text{tot} \equiv \int \rho_\text{DF}(R,|z| \mid \pmodel) \cdot \text{SF}(\vect{x}) \Diff 3 x.\label{eq:normalisation}
\end{equation}
In the case of an axisymmetric Galaxy model and $\text{SF}(\vect{x})=1$ within the observation volume (as in most tests in this work), the normalization is essentially a two-dimensional integral in the $(R,z)$ plane over $\rho_{DF}$ with finite integration limits. We evaluate the integrals using Gauss-Legendre quadratures of order 40. The integral over the azimuthal direction can be solved analytically. 
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/isoSphFlexErrConv_MC_vs_error_3-eps-converted-to.pdf}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.7\textwidth]{figs/isoSphFlex_short_hot_2kpc_triangle_MCMC-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/isoSph_CLT_2-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/wedgesVol_bias_vs_SE-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/MWbdhIncompR_violins-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/isoSphFlexErrConv_SE_vs_error-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/isoSphFlexErrConv_bias_vs_SE-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/isoSphFlexErrSyst_offset_vs_error-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/MWdhbMix_mockdata_residuals-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/MWbdhMixDiff_hot_2kpc_0.6_a_residual_RvsZ-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[scale=0.55]{figs/MWbdhMixCont_violins-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[scale=0.55]{figs/MWbdhMixDiff_violins-eps-converted-to.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{figs/MW14vsKKS2New_mockdata_residuals-eps-converted-to.pdf}
...
\begin{figure*}[!htb]
\centering
\includegraphics[width=1\textwidth]{figs/MW14vsKKS2New_b_contours_compare_withsurfdens_2-eps-converted-to.pdf}
...
%====================
\begin{figure*}[!htbp]
\includegraphics[width=1\textwidth]{figs/MW14vsKKS2New_violins-eps-converted-to.pdf}
...
\tabletypesize{\scriptsize}
\tablecaption{Summary of Test Suites in this Work. \label{tbl:tests}}
\tablewidth{0pt}
\tablehead{
\colhead{Test} & & \colhead{Model for Mock Data} & \colhead{Model in Analysis} & \colhead{Figures and Results}}
\startdata
...
normalization & & & & required for cooler populations.\\
\tableline
...
of convolution		& \emph{Survey Volume:}	& sphere around Sun, $r_\text{max} = 3~\text{kpc}$ & (fixed and known) & scales as $N_\text{samples} \propto (\delta v_\text{max})^2$,\\
with measurement		& \emph{Uncertainties:}		& $\delta \text{R.A.} =\delta \text{decl.} =\delta(m-M)=0$	& (fixed and known)	& with $\delta v_\text{max}$ being the largest $\delta v$\\
uncertainties					&						& $\delta v_\text{los} = 2~\text{km s}^{-1}$ & & in the data set. If $N_*$ is smaller,\\
						&						& $\delta \mu =$ 2,3,4, or $5~\text{mas yr}^{-1}$ & & less accuracy, i.e., less $N_\text{samples}$,\\
...
parameters' \pdf{} & \emph{Survey Volume:} & sphere around Sun, $r_\text{max} = 2~\text{kpc}$ & (fixed and known) & Gaussian in the limit of\\
...
Influence of survey & \emph{Survey volume:} & sphere around Sun, $r_\text{max} = 0.2, 1, 2, 3,$ or $4~\text{kpc}$ & (fixed and known) & estimator. Larger survey volumes\\
...
Influence of 			& 						& 													& (ii) \texttt{DHB-Pot}, free parameters: & The exact position and shape of the\\
position and shape 		& 			& 										& $v_\text{circ}(R_\odot)$, $a_\text{disk}$, $f_\text{halo}$ & survey volume plays only a\\
of survey volume 		& \emph{DF:} & \texttt{hot} qDF											& qDF, all parameters free & minor role. Having both large\\
on parameter recovery 	& \emph{Survey volume:}	& four different wedges, see Figure \ref{fig:wedgesVol_bias_vs_SE}, upper panel & (fixed and known) & radial and vertical extent should\\
...
about the spatial SF & \emph{Survey volume:} & sphere around Sun, $r_\text{max} = 3~\text{kpc}$ & (fixed and known) & (i.e., $\epsilon_r \lesssim 0.15$ for \texttt{hot} and\\
...
uncertainties on & \emph{Survey volume:} & sphere around Sun, $r_\text{max} = 3~\text{kpc}$ & (fixed and known) & precision of the potential para-\\
precision of & \emph{Uncertainties:} & (i) $\delta\text{R.A.}=\delta\text{decl.}=\delta(m-M)=0,$ & (fixed and known) & meter recovery is only a\\
...
with measurement 		& \emph{Survey Volume:}	& sphere around Sun, $r_\text{max} = 3~\text{kpc}$ & (fixed and known) & in Equation \eqref{eq:errorconv} is the true\\
 uncertainties in Equation \eqref{eq:errorconv} & \emph{Uncertainties:}		& $\delta \text{R.A.} =\delta \text{decl.} =0$,	& (fixed and known)	& likelihood in the absence of\\
with and without			&						& $\delta v_\text{los} = 2~\text{km s}^{-1}$, & & position errors. For distance\\
distance uncertainties						&						& $\delta \mu =$ 1,2,3,4, or $5~\text{mas yr}^{-1}$, & & uncertainties $\lesssim10\%$ at\\
						...
						& & (ii) $\delta(m-M) \neq 0$ (see Figure \ref{fig:isoSphFlexErrConv_bias_vs_SE}) & & for \emph{Gaia} and $G \lesssim 15$, the intro-\\
...
\tableline
Test \testlabel{test:MW14vsKKS2New}{8} :			& \emph{Potential:} & \texttt{MW14-Pot} & \texttt{KKS-Pot}, all parameters free, & Figures \ref{fig:MW14vsKKS2New_mockdata_residuals}, \ref{fig:MW14vsKKS2New_contours}, and \ref{fig:MW14vsKKS2New_violins}\\
...
from the stars'			& \emph{Survey volume:} & sphere around Sun, $r_\text{max} = 4~\text{kpc}$ & (fixed and known) & where most stars are located,\\
...
\enddata
\tablecomments{Reference potentials and qDFs are introduced in Tables \ref{tbl:referencepotentials} and \ref{tbl:referenceMAPs}, respectively. Parameters that are not left free in the analysis, are always fixed to their true value. Unless stated otherwise, all mock data sets have SFs with completeness$(\vect{x}) = 1$ and no measurement uncertainties, and we use $N_x=16, N_v = 24, n_\sigma = 5$ as numerical accuracy for calculating the likelihood normalization (see Section \ref{sec:likelihood_normalisation}).\\The first column indicates the test suite; the second column the potential, DF, and SF model, etc., used for the mock data creation; the third column the corresponding model assumed in the \RM{} analysis; and the last column lists the figures belonging to the test suite and summarizes the results.}
\end{deluxetable}
\clearpage
\end{landscape}
...
\centering
\includegraphics[width=\textwidth]{figs/kks2WedgeEx_mockdata_actions-eps-converted-to.pdf}
...
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{figs/kks2WedgeEx_mockdata_xv-eps-converted-to.pdf}
...
We set $0 \leq \text{completeness}(\vect{x}) \leq 1$ everywhere inside the observed volume, so it can be understood as a position-dependent detection probability for a star at $\vect{x}$. Unless explicitly stated otherwise, we simplify to $\text{completeness}(\vect{x}) = 1$. Additionally, we use in Figure \ref{fig:wedgesVol_bias_vs_SE} (Test \ref{test:wedgesVol} in Table \ref{tbl:tests}) and Figures \ref{fig:kks2WedgeEx_actions}-\ref{fig:kks2WedgeEx_xv} for illustrative purposes some rather unrealistic survey volumes which are angular segments of a cylindrical annulus (wedge), i.e., the volume with $R \in [R_\text{min},R_\text{max}],\phi \in [\phi_\text{min},\phi_\text{max}],z \in [z_\text{min},z_\text{max}]$ within the model Galaxy. 
