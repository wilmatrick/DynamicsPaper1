...
%[TO DO: Delete these makros for final version. ?????????????????????????????????]
...
\Wilma{[TO DO: Delete Wilma makro]}
...
\NOTE{[The figure demonstrating that the width of the pdf scales as $1/\sqrt{N_*}$ was removed. (TO DO: Remove this note for the final version.)]}
%-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
\NOTE{[I removed the Figure, that showed the \texttt{Iso-Pot} analyses with marginalization of the likelihood over $v_T$ and I also did not replace it with a new figure using the \texttt{DHB-Pot}. The reason is, that the suspiciously robust recovery of the Isochrone scale length $b$ was not the case for the \texttt{DHB-Pot} parameters and I therefore did not need this plot anymore. (TO DO: remove this note for final version)]}
...
\NOTE{[The reference list is now generated using Bibtex and the style file aasjournal. Hopefully this makes the format and journal abbreviations correct. (TO DO: remove this note for the final version.)]}
...
\enddata
\end{deluxetable}
\NOTE{[I removed two tests from this table w.r.t. the original version: The test that shows how the width of the pdf scales by $1/\sqrt{N_*}$, and the description of Figures \ref{fig:kks2WedgeEx_xv} and \ref{fig:kks2WedgeEx_actions}, which have been moved to the appendix. (TO DO: remove this note for final version.)]}
\clearpage
\end{landscape}
