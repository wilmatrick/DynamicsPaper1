...
\caption{Relative error of the likelihood normalization in Equation \ref{eq:relerrlikelihood} depending on the accuracy of the density calculation in Equation \ref{eq:tracerdensity} (and surrounding text). The different colors represent calculations for different spherical observation volumes with radius $r_\text{max}$ as indicated in the legend. $2 r_\text{max} / N_x$ is the spatial resolution in $R$ of the regular grid points on which the tracer density is evaluated according to Equation \ref{eq:tracerdensity}. (As the tracer scale height is much smaller than the tracer scale length, we used twice this resolution, i.e. $r_\text{max} / N_x$, in $z$.) At each $(R,z)$ a Gauss-Legendre integration of order $N_v$ is performed over an integration range of $\pm n_\sigma$ times the velocity dispersion in $v_R$ and $v_z$ and $[0,1.5v_\text{circ}(R_\odot)]$ in $v_T$. To integrate the interpolated density over the observed volume to arrive at the likelihood normalization in Equation \ref{eq:normalisation}, we perform a 40th-order Gauss-Legendre integration in each $R$ and $z$ direction. We compare the convergence of the normalisation for the \texttt{hot} qDF in three potentials, \texttt{Iso-Pot}, \texttt{MW13-Pot} and \texttt{KKS-Pot} (see also Test \ref{test:norm_accuracy} in Table \ref{tbl:tests} for all other model details). In each column of plots we keep two of the accuracy parameters fixed (indicated on top), while the third parameter is varied. We calculate the true normalization with high accuracy as $M_\text{tot,true} \approx M_\text{tot}(N_x=20,N_v=56,N_\sigma=7)$. The dashed lines indicate the accuracy used in our analyses: it is better than $0.002\%$ for all three potential types. Only for the smallest volume in the \texttt{MW13-Pot} (yellow line) the error is only $\sim 0.005\%$. This could be due to the fact, that, while we have analytical formulas to calculate the actions for the isochrone and the Staeckel potential exactly, we have to resort to an approximate action calculation for the MW-like potential (see Section \ref{sec:potentials}). [TO DO: Try to redo yellow curve in MW. Weird, that it does not depend on $N_{spatial}$.???] [TO DO: fancybox Legend] [TO DO: Potential and/or population names in typewriter font] [TO DO: I completely changed this plot. Also change the whole caption, test table, text etc.] [TO DO: Check plot again... It's weird for $N_x$ being in the denominator, and $N_v$ in the numerator... ]}
\end{figure*}
