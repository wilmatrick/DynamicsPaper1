...
\tableline
Test \testlabel{test:norm_accuracy} {2}:         & \emph{Potential:}     & \texttt{KKS-Pot} & - & Convergence\\
...
with measurement		& \emph{Uncertainties:}		& $\delta \text{RA} =\delta \text{DEC} =\delta(m-M)=0$	& Convolution with	& \\
uncertainties					&						& $\delta v_\text{los} = 2$ km/s	& perfectly known & \\
						&						& $\delta \mu_\text{RA}= \delta \mu_\text{DEC}  =$ 2,3,4 or 5 mas/yr & measurement uncertainties & \\
...
with measurement \& without  & \emph{Uncertainties:}		& $\delta \text{RA} =\delta \text{DEC} =0$,	& Convolution with	& \\
uncertainties with			&						& $\delta v_\text{los}  = 2~\text{km s}^{-1}$, & measurement uncertainties,  & \\
distance errors						&						& $\delta \mu_\text{RA} = \delta \mu_\text{DEC} =$ 1, 2,3,4 or 5 mas/yr, & ignoring distance errors in position& \\
						&						& a) $\delta(m-M) = 0$  or & (see Section \ref{sec:likelihood})&  \\
						&                     & b) $\delta(m-M) \neq 0$.  & & \\
						&                     &  (see Figure \ref{fig:isoSphFlexErrConv_bias_vs_SE})  & & \\
						& \emph{Numerical Accuracy:} & & $N_\text{error} = 800$ for $\delta \mu \leq 3 \text{mas yr}^{-1}$, & \\
						&                     & & $N_\text{error} = 1200$ for $\delta \mu > 3 \text{mas yr}^{-1}$& \\
						& \emph{\# stars per data set:} & 10,000 & & \\
\tableline
...
errors 			 	& \emph{Uncertainties:}		& only proper motion errors & Convolution with proper motion errors & \\
...
\Wilma{[TO DO: $N_* = $ in front of number of stars]}
